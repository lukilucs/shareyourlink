...
import GetDocForm from "@/components/interface-docs/get-docs-form";

interface GetDocPageProps {
  params: Promise<{ locale: string }>;
}

export async function generateMetadata({
  params,
}: GetDocPageProps): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "GetDocPage" });
...
const GetDocPage = async () => {
  const t = await getTranslations("GetDocPage");
