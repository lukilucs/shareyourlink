...
async function validateDocFile(
  file: File,
  ...
  const hasExpectedSignature =
    (extension === "pdf" && startsWithSignature(fileBytes, PDF_SIGNATURE)) ||
    (extension === "doc" && startsWithSignature(fileBytes, OLE_SIGNATURE)) ||
    (extension === "docx" && startsWithSignature(fileBytes, ZIP_SIGNATURE));
...
export async function createSharedDoc(
  ...
  const validatedBuffer = await validateDocFile(file, extension);
  if (!validatedBuffer) {
    return {
      success: false,
      error: "The uploaded file does not match the expected document format",
    };
  }
...
export async function getDocByCode(
  ...
  if (!entry || !entry.fileKey.startsWith("docs/")) {
